...
class EmpPortalTimeOff(models.Model):
    ...
    def create_timeoff_portal(self, values):
        ...
        dura_days = int(dura_days['days']) + 1
        ...
        if (tftype.custom_leave == True and tftype.applicable_for == 'both' and tftype.only_time_granted == False):
            if emp.gender == 'male' or emp.gender == 'female':
                if dura_days > tftype.fixed_period:
                    raise ValidationError(_("Duration must be ") + str(tftype.fixed_period))
        ...
        # for wedding and hej
        if (tftype.custom_leave == True and tftype.applicable_for == 'both' and tftype.only_time_granted == True):
            if emp.gender == 'male' or emp.gender == 'female':
        ...
        # for women only
        if tftype.custom_leave == True and tftype.applicable_for == 'female':
    ...
    def number_of_days_portal(self, values):
        ...
        if (values['start'] and values['end']) and values['timeoff_type']:
